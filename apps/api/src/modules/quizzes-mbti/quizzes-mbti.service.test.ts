...
describe('QuizzesMbtiService', () => {
  ...
  beforeEach(() => {
    ...
    service = new QuizzesMbtiService(quotasService as QuotasService, walletEngine as WalletEngineService);
  });
  ...
  it('trả kết quả hợp lệ + gọi quota khi MBTI bật + free-for-all', async () => {
    ...
    expect(quotasService.assertCanExecute).toHaveBeenCalledWith('mbti-quiz', user.userId, '127.0.0.1', false);
  });
  ...
  it('coi email rỗng là phiên ẩn danh (truyền isAnonymous=true cho quota)', async () => {
    ...
    expect(quotasService.assertCanExecute).toHaveBeenCalledWith('mbti-quiz', anon.userId, '127.0.0.1', true);
  });
});
