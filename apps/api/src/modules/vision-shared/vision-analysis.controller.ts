...
const MISSING_IMAGE_MESSAGE: Record<VisionKind, string> = {
  ...
  tarot: 'Vui lòng tải lên một tấm ảnh lá bài Tarot.',
};

/**
 * POST /vision/face (US-017e) + POST /vision/palm (US-017f) + POST /vision/tarot: luận giải từ ảnh.
 *
 * Các route cùng một khuôn multipart/form-data: field `image` (1 ảnh ≤ 4MB) + `question` (tuỳ chọn).
...
export class VisionAnalysisController {
  ...
  @UseInterceptors(RequireXU(10), FileInterceptor('image', { limits: { fileSize: MAX_IMAGE_BYTES } }))
  async analyzeFace(
  ...
  async analyzeTarot(
    ...
    return this.analyze('tarot', currentUser, request, file, question);
  }
