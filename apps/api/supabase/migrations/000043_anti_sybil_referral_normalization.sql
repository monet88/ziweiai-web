-- ==============================================================================
-- Migration 000043: Anti-Sybil Referral Normalization & Alias Protection
-- Tăng cường bảo mật: Chặn tự giới thiệu (Self-referral) qua Gmail alias / dot trick
-- và từ chối phát thưởng người giới thiệu nếu người được mời (referee) dùng email alias (+).
-- ==============================================================================
...
begin
  ...
  if v_referrer_id is not null and v_referrer_id <> p_user_id then
    if not exists (select 1 from public.referrals where referee_id = p_user_id) then
      ...
      -- Chuẩn hóa email chống Sybil attack:
      -- 1. Giới hạn trần 10 ref/ngày
      -- 2. Chặn tự giới thiệu bất kể biến thể dấu chấm hay dấu + của Gmail
      -- 3. Người được mời dùng email chứa alias dấu (+) sẽ không tạo hoa hồng cho người giới thiệu
      if v_ref_today_count < 10 
         and v_referrer_email is not null 
         and trim(v_referrer_email) <> '' 
         and public.normalize_email_address(v_referrer_email) <> public.normalize_email_address(v_user_email)
         and position('+' in split_part(v_user_email, '@', 1)) = 0 then
