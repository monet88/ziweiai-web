...
/**
 * POST /vision/face (US-017e): luận giải Xem Tướng từ ảnh chân dung.
 *
 * multipart/form-data: field `image` (1 ảnh ≤ 4MB) + `question` (tuỳ chọn). Controller chỉ validate
 * đầu vào (INVALID_INPUT) rồi uỷ quyền VisionAnalysisService (gate + upload + vision LLM). multer
 * memoryStorage → file.buffer in-memory (không ghi đĩa); ảnh chỉ lưu Storage qua gateway.
 */
@Controller('vision/face')
export class VisionFaceController {
  ...
  async analyze(
    ...
    if (!file) {
      throw new ApiErrorHttpException(
        ...
        'INVALID_INPUT',
        'Vui lòng tải lên một tấm ảnh chân dung.',
