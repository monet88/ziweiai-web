...
describe('DreamsService', () => {
  ...
  beforeEach(() => {
    ...
    service = new DreamsService(quotasService as QuotasService, providerRouter as ExplanationProviderRouter, walletEngine as WalletEngineService);
  });
  ...
  it('trả payload hợp lệ + khớp biểu tượng + gọi quota khi bật + free-for-all', async () => {
    ...
    expect(quotasService.assertCanExecute).toHaveBeenCalledWith('dream-reading', user.userId, '127.0.0.1', false);
  });
  ...
  it('dùng quota anon khi user không có email', async () => {
    ...
    expect(quotasService.assertCanExecute).toHaveBeenCalledWith('dream-reading', anon.userId, '10.0.0.1', true);
  });
