...
// → bàn 12 cung tiếng Việt → chọn cung → luận giải (backend + provider AI thật) → markdown
// VN render sanitized → tạo lá số thứ hai → cung selected reset đúng (remount {#key chartId}).
//
...
test('US-006: tạo lá số → 12 cung VN → chọn cung → luận giải → reset khi đổi lá số', async ({
  ...
  // ---- Luận giải cung đang chọn (backend + provider AI thật) ----
  ...
  // nút đổi nhãn "Luận giải cung này" → "Tạo lại luận giải" và article kết quả hiện ngay. Bấm nút
  // luận giải DÙ ở trạng thái nào (tạo mới hoặc tạo lại) — cả hai đều gọi backend + provider thật.
  await page
    .getByRole('button', { name: /^(Luận giải cung này|Tạo lại luận giải)$/ })
    .click();

  // Kết quả markdown hiện ra trong <article class="result"> → MarkdownView. Provider mạng
  // thật có thể chậm → chờ tối đa 60s. Nếu provider chết/quota: test fail (đúng — STOP báo lỗi).
  ...
  await expect(
    page.getByRole('main').getByRole('button', { name: 'Lập lá số', exact: true }),
  ).toBeVisible();
